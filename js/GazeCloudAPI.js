...
//--------------------
	...
	function __ProcessWebRecStream(bend = false) {
...
//    var MaxBufforSize = 2000;
	...
	function _ShowCalibration() {
		...
		if (true) //head move
		{
			var showtimeHead = showtime * 1.3;
			var headmove = .3;
			vPoints.push({
				x: .5,
				y: .5,
				color: "white",
				txt: "Turn your head slightly in the direction of the arrow",
				type: -1,
				time: infotime,
				head: null //document.getElementById('arrowleft')
			});
			vPoints.push({
				x: .5 - headmove,
				y: .5,
		...
		_LoopCalibration = setInterval(() => {
			...
			try {
				...
				if (Ix > 0 && vPoints[Ix].type == 1) // move point
				{
					////////////////// move animation/////////////////
					var f = 1.0 - size;
					f = size * size; // Math.sqrt(size);;//-2;
					// f = 1.0 - Math.sqrt(size);
					//f = .5;
	...
	function InitClickCalibration() {
		...
		document.onmousedown = GazeCloudAPI.processClick;
		return;
		var cursorX;
		var cursorY;
		...
		if (true) {
			...
			if (videoTime <= LastVideoTime) {
				...
				if (dd > 500) // frozen min 
				{
					...
					if(false)//tmp
					if(LastVideoTime ==_CurVideoTime  )
					{

					 setTimeout(function(){requestAnimationFrame(CamSendLoop);}, 10);
					return;
					}
...
window.addEventListener("DOMContentLoaded", function() {
	processClick = GazeCloudAPI.processClick;
});
/////////end Version 1.0.0///////////